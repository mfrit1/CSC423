...
$sqlQuery2 = "SELECT inventoryitem.itemId, inventoryitem.description, inventoryitem.size, inventoryitem.division, inventoryitem.department, inventoryitem.category, inventoryitem.itemCost, inventoryitem.itemRetail, inventoryitem.imageFileName, inventoryitem.vendorId, inventory.quantityInStock 
		FROM inventoryitem, inventory, retailstore WHERE ((inventoryitem.status='Active') 
		AND (inventory.itemId = inventoryitem.itemId) AND (retailstore.storeCode ='{$id}') AND (retailstore.storeId = inventory.storeId)) ORDER BY inventoryitem.itemId";
	...
	if(isset($_POST["check"])){
		...
		if(isset($_POST["checkQuantity"])){
			$quantity = mysqli_escape_string($conn, $_POST["checkQuantity"]);
			$sql = "SELECT inventoryitem.itemId, inventoryitem.description, inventoryitem.size, inventoryitem.division, inventoryitem.department, 
			inventoryitem.category, inventoryitem.itemCost, inventoryitem.itemRetail, inventoryitem.imageFileName, inventoryitem.vendorId, inventory.quantityInStock FROM inventoryitem, inventory, retailstore 
			WHERE ((inventoryitem.status='Active') AND (inventory.itemId = inventoryitem.itemId) AND (retailstore.storeCode ='{$id}') AND (retailstore.storeId = inventory.storeId) AND (inventory.quantityInStock > {$quantity})) ORDER BY inventoryitem.itemId";
		}
		else
			$sql = $sqlQuery2;
	...
	else if(isset($_POST["checkQuantity"])){
		$quantity = mysqli_escape_string($conn, $_POST["checkQuantity"]);
		$sql = "SELECT inventoryitem.itemId, inventoryitem.description, inventoryitem.size, inventoryitem.division, inventoryitem.department, 
		inventoryitem.category, inventoryitem.itemCost, inventoryitem.itemRetail, inventoryitem.imageFileName, inventoryitem.vendorId, inventory.quantityInStock FROM inventoryitem, inventory, retailstore 
		WHERE ((inventoryitem.status='Active') AND (inventory.itemId = inventoryitem.itemId) AND (retailstore.storeCode ='{$id}') AND (retailstore.storeId = inventory.storeId) AND (inventory.quantityInStock > {$quantity})) ORDER BY inventoryitem.itemId";
